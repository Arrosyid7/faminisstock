 ...
 */
function tambahProduk(idProduk, namaProduk, kategori, pengguna, stokMinimum) {
  if (ambilPeranPengguna(pengguna) !== 'master') {
    return { sukses: false, pesan: 'Hanya Admin Master yang boleh menambah produk.' };
  }
  idProduk = (idProduk || '').toString().trim().toUpperCase();
  if (!idProduk || !namaProduk) {
    return { sukses: false, pesan: 'ID Produk dan Nama Produk wajib diisi.' };
  }
  ...
  try {
    lock.waitLock(10000);
    const ss = getDb();
    pastikanSheetProdukSiap(ss); // pastikan kolom Stok_Total & Stok_Minimum sudah ada dulu
    const sheet = ss.getSheetByName('Produk');
    const data = sheet.getDataRange().getValues();
    for (let i = 1; i < data.length; i++) {
      if (data[i][0] && data[i][0].toString().trim().toUpperCase() === idProduk) {
        return { sukses: false, pesan: `ID Produk ${idProduk} sudah terdaftar.` };
      }
    }

    const lastCol = sheet.getLastColumn();
    const header = sheet.getRange(1, 1, 1, lastCol).getValues()[0].map(h => h.toString().trim());
    const nilaiKolom = {
      'ID_Produk': idProduk,
      'Nama Produk': namaProduk,
      'Kategori': kategori || '',
      'Stok_Total': '', // dibiarkan kosong -- akan diisi rumus SUMIF oleh pastikanFormulaStokTotal()
      'Stok_Minimum': Number(stokMinimum) || 0
    };
    // Kolom lain di luar daftar ini (mis. "Harga Ecer"/"Harga Grosir" peninggalan versi lama)
    // sengaja dibiarkan kosong ('') untuk baris baru -- lihat catatan di atas.
    const barisBaru = header.map(function (h) { return (h in nilaiKolom) ? nilaiKolom[h] : ''; });
    sheet.appendRow(barisBaru);

    pastikanSemuaSheetStokSiap(ss);
    pastikanFormulaStokTotal(ss);

    return { sukses: true, pesan: `Produk ${idProduk} berhasil ditambahkan!` };
  } catch (e) {
    return { sukses: false, pesan: e.message || 'Sistem sibuk, coba lagi.' };
  } finally {
    lock.releaseLock();
  }
}
...
/**
 * Mengubah data produk yang SUDAH ADA (Nama, Kategori, Harga Ecer/Grosir, Stok Minimum),
 * khusus Admin Master. Dipakai halaman "Kelola Produk" untuk mengganti skema penamaan lama
 * (misal berbasis kode motif) menjadi nama per jenis item, atau sekadar mengatur/mengubah
 * ambang batas Stok Minimum, tanpa perlu buka Spreadsheet langsung.
 * Stok_Total TIDAK bisa diedit lewat sini karena itu rumus otomatis (SUMIF), bukan angka manual.
 * Produk TIDAK lagi punya "harga katalog" -- kolom "Harga Ecer"/"Harga Grosir" peninggalan versi
 * lama (kalau masih ada di sheet) sengaja tidak disentuh/diubah lagi lewat sini.
 */
function perbaruiProduk(idProduk, namaProduk, kategori, stokMinimum, pengguna) {
  if (ambilPeranPengguna(pengguna) !== 'master') {
    return { sukses: false, pesan: 'Hanya Admin Master yang boleh mengubah data produk.' };
  }
  idProduk = (idProduk || '').toString().trim().toUpperCase();
  if (!idProduk || !namaProduk) {
    return { sukses: false, pesan: 'ID Produk dan Nama Produk wajib diisi.' };
  ...
  try {
    ...
    let baris = -1;
    for (let i = 1; i < data.length; i++) {
      if (data[i][0] && data[i][0].toString().trim().toUpperCase() === idProduk) { baris = i + 1; break; }
    }
    if (baris === -1) {
      return { sukses: false, pesan: `Produk ${idProduk} tidak ditemukan.` };
    }

    const kolomNama = cariKolom(sheet, 'Nama Produk');
    const kolomKategori = cariKolom(sheet, 'Kategori');
    const kolomMinimum = cariKolom(sheet, 'Stok_Minimum');

    if (kolomNama) sheet.getRange(baris, kolomNama).setValue(namaProduk);
    if (kolomKategori) sheet.getRange(baris, kolomKategori).setValue(kategori || '');
    if (kolomMinimum) sheet.getRange(baris, kolomMinimum).setValue(Number(stokMinimum) || 0);

    // Nama produk ikut disinkronkan ke semua sheet Stok_xxx supaya label tampilannya konsisten
    // (kolom "Nama Produk" di sheet Stok_xxx cuma label, tidak dipakai untuk perhitungan apapun).
    const daftarLokasi = ambilDaftarLokasi();
    daftarLokasi.forEach(function (lok) {
      ...
      for (let j = 1; j < dataStok.length; j++) {
        if (dataStok[j][0] && dataStok[j][0].toString().trim() === idProduk) {
          sheetStok.getRange(j + 1, 2).setValue(namaProduk);
          break;
        }
      }
    });

    return { sukses: true, pesan: `Produk ${idProduk} berhasil diperbarui!` };
  } catch (e) {
    return { sukses: false, pesan: e.message || 'Sistem sibuk, coba lagi.' };
  } finally {
    lock.releaseLock();
...
/**
 * Daftar lengkap semua produk (dipakai halaman "Kelola Produk" khusus Admin Master), termasuk
 * Stok_Minimum & Stok_Total saat ini. Kolom dicari lewat NAMA HEADER (bukan posisi tetap) supaya
 * aman dari perbedaan urutan kolom antar spreadsheet lama & baru.
 */
function ambilDaftarProduk() {
  ...
  const idx = {
    ...
    total: header.indexOf('Stok_Total'),
    minimum: header.indexOf('Stok_Minimum')
  };

  const hasil = [];
  for (let i = 1; i < data.length; i++) {
    const idProduk = data[i][0] ? data[i][0].toString().trim() : '';
    if (!idProduk) continue;
    hasil.push({
      idProduk: idProduk,
      namaProduk: idx.nama > -1 ? data[i][idx.nama] : '',
      kategori: idx.kategori > -1 ? data[i][idx.kategori] : '',
      stokTotal: idx.total > -1 ? (parseFloat(data[i][idx.total]) || 0) : 0,
      stokMinimum: idx.minimum > -1 ? (parseFloat(data[i][idx.minimum]) || 0) : 0
    });
  }
  return hasil;
}
